...
/*dotValidator
Original Author: Nathan J Plummer*/
...
//dot color on page load
//equal to No Input by Default
dotDefault(dotNoInput);
...
inputCode.textarea = (function(targ) {
  if (targ.value === "") {
    targ.nextElementSibling.style.backgroundColor = dotInvalid;
  } else {
    targ.nextElementSibling.style.backgroundColor = dotValid;
...
function validateMe(targ) {
  ...
  } else if (targ.type === "textarea");
    inputCode.textarea(targ);
  };
...
function dotDefault(color) {
  ...
  for (i = 0; i < dots.length; i++) {
    //hide dot if user disables it
    if (dots[i].previousElementSibling.dataset.dotvalidator === "false") {
